...
{
	int d; 
	for (d = maxd; d >= -1; d--) if (f[d] != 0) break;
	return d;
}
...
int polrootsmod(int* f, int degf, int* roots, int p)
{
	...
	while (degf1 > 0) {
		...
		if (degf2 > degf1) {	// make f1 'largest'
			polsetzero(f1, degf);
			polsetzero(f2, degf);
			for (int i = 0; i < degf2; i++) { int t = f1[i]; f1[i] = f2[i]; f2[i] = t; }
			int degt = degf1; degf1 = degf2; degf2 = degt;
		}
		// make f2 monic
		int f2d = f2[degf2];
		int f2dinv = modinv(f2d, p);
	...
	while ((!xdivg && k < 2*degg) || (xdivg && k < 2*degg-1)) { 
		// compute (x + a)^((p-1)/2) (mod g, p)
		polsetzero(r, degg);
		polsetzero(r2, degg*2);
		r[1] = 1; r[0] = a;
		mask = 1;
		while (mask << 1 <= (p-1)>>1) mask <<= 1;
		while (mask) {
			// square
			for (int j = 0; j < degf; j++) {
				for (int i = 0; i < degf; i++) {
		...
		int degh1 = poldegree(h1, degg);
		if (degh1 > 1 && degh1 < degg) {	// found a factor!
			// divide g by h1 to get other factor
			polsetzero(h2,degg); polsetzero(r, degf*2+1);
			for (int i = 0; i <= degg; i++) r[i] = g[i];
			int c = modinv(h1[degh1], p);
			int degr = poldegree(r, degg);
			while (degr >= degh1) {
				int s = mod(r[degr] * c, p);
				h2[degr - degh1] = mod(h2[degr - degh1] + s, p);
				for (int i = 0; i <= degh1; i++) {
					r[degr - i] = mod(r[degr - i] - s * h1[i], p);
				}
				degr = poldegree(r, degg);
			}
			// expand list of known factors
			k++;
			// shift start positions of factors after gidx[i0] on by one
			for (int i = degg; i > i0; i--) gidx[i] = gidx[i-1];
			// insert new gidx position
			gidx[i0+1] = gidx[i0] + degh1;
			// shift every coeff after position gidx+degg on by one (to make room for new factors)
			for (int i = k; i > degg; i--) gsplit[i] = gsplit[i-1];
			// replace coeffs of g by those of the two new non-trivial factors
			for (int i = 0; i <= degh1; i++) gsplit[gidx[i0] + i] = h1[i];
			// add cofactor
			int degh2 = degg - degh1;
			for (int i = 0; i <= degh2; i++) gsplit[gidx[i0] + degh1 + 1 + i] = h2[i];
		}
		if ((g[0] != 0 && k < 2*degg) || (g[0] == 0 && k < 2*degg-1)) {
			// select first nonlinear factor
			while (gidx[i0+1] - gidx[i0] == 2) i0++;
			// store in g
			int d = gidx[i0+1] - gidx[i0];
			for (int i = 0; i <= d; i++) g[i] = gsplit[gidx[i0] + i];
			// advance to next a
			a++; if (a == p) a = 1;
		}
	}
