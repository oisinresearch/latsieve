...
int latsieve2d(int64_t skew, int64_t* f, int degfq, int degfp, int64_t q, int l, int* allp, int nump, int* s, int* num_smodp,
...
int main(int argc, char** argv)
{
	...
	int cofmax = 1500;	//int cofmaxbits = 52;
	if (argc >= 12) cofmax = atoi(argv[11]);
	...
	while (q < qmax) {
		...
		cout << "..." << endl;
		start = clock();
		int m = latsieve2d(skew, fq, degfside, degf, q, 0, sievep0, k0, sieves0, sievenum_s0modp, M, Mlen, B);
		timetaken = ( clock() - start ) / (double) CLOCKS_PER_SEC;
		...
		cout << "..." << endl;
		start = clock();
		m = latsieve2d(skew, fq, degfside, degg, q, 0, sievep1, k1, sieves1, sievenum_s1modp, M, Mlen, B);
		timetaken = ( clock() - start ) / (double) CLOCKS_PER_SEC;
		cout << "# Finished! Time taken: " << timetaken << "s" << endl;
		cout << "# Size of lattice point list is " << m << "." << endl;
		cout << "# Constructing histogram..." << endl;
