...
int main(int argc, char** argv)
{
	...
	mpz_t S; mpz_init(S); GetlcmScalar(cofacS, S, primes, 669); // 1229); // max S = 10000 // 669);	// max S = 5000
			 ...
			 keyval* M, int Mlen, int* B)
{
	...
	while (i < nump) {
		...
		int128_t rl = mod(-r[l], q);
		for (int k = 0; k < num_smodp[i]; k++) {
			int128_t n = P*Q;
			int128_t sk = static_cast<int128_t>(mod(-s[i*degf+k],p));
			int128_t t =   Q*static_cast<int128_t>( (sk * modpinvq[i]) % p )
                         + P*static_cast<int128_t>( (rl * modqinvp[i]) % q ); // CRT
			if (t >= n) t -= n;
			L2[0] = n; L2[1] = t; L2[2] = 0;
			L2[3] = 0; L2[4] = 1; L2[5] = t;
			L2[6] = 0; L2[7] = 0; L2[8] = 1;
			int128L2(L2,3);	// LLL reduce L, time log(n)^2
			mat3x3prod(qLinv, L2, L3);	// L3 =  qLinv*L2
            for (int ii = 0; ii < 9; ii++) L3[ii] /= Q;
            int128L2(L3,3);
			int u1 = L3[0].convert_to<int>(); int u2 = L3[3].convert_to<int>(); int u3 = L3[6].convert_to<int>();
			int v1 = L3[1].convert_to<int>(); int v2 = L3[4].convert_to<int>(); int v3 = L3[7].convert_to<int>();
			int w1 = L3[2].convert_to<int>(); int w2 = L3[5].convert_to<int>(); int w3 = L3[8].convert_to<int>();
